...
export const exportFile = async (file: string): Promise<void> => {
  ...
  const command = `git diff --src-prefix=a/ --dst-prefix=b/ --full-index ${resolve(ENGINE_DIR, file)}`
  const proc = execSync(command, { cwd: ENGINE_DIR, encoding: 'utf-8' })
  ...
  writeFileSync(resolve(SRC_DIR, ...patchPath, name), proc)
